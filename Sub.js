...
[Proxy Group]
Nexitally = select, policy-path=http://sub.info?url=机场节点链接&reset_day=1&alert=1
...
(async () => {
  ...
  let infoList = [`Usage: ${bytesToSize(used)} | ${bytesToSize(total)}`];
  
  if (resetLeft) {
    infoList.push(`Traffic Reset: ${resetLeft}天`);
  }
  if (expire) {
    if (/^[\d]+$/.test(expire)) expire *= 1000;
    infoList.push(`Expire Date: ${formatTime(expire)}`);
  }
